...
class LSTD:
    ...
    def run_solver(self, policy_prob):
        ...
        for sample in data:
            ...
            phi_sa_next_ids = set(
                self.feature.get_feature(sample.next_s, sample.next_a)
            )
    ...
    def get_estimated_Q(self, state):
        ...
        q = [
            np.sum(
                np.asarray(
                    [self.weights[k] for k in self.feature.get_feature(state, a)]
                ),
                axis=0,
            )
    ...
    def get_error_norm(self, trueQ, estimatedQ):
        ...
        """
        return np.linalg.norm(trueQ - estimatedQ, ord="fro") / np.linalg.norm(
            trueQ, ord="fro"
        )
...
class Sampling:
    ...
    def __init__(self, env, num_samples, tc_args, constraints=True):
        ...
        self.constraints = (
            constraints  # whether the env has constraints or not (mdp/cmdp)
        )
...
class KW_Sampling:
    ...
    def get_data(self, policy_prob):
        ...
        for k in range(self.num_samples * self.len_index):
            list_s_a_pairs = self.index_to_sa_pairs[
                np.random.choice(self.list_of_indexes)
            ]
    ...
    def get_data(self, policy):
        ...
        for t in range(self.num_traj):
            ...
            while running:
                ...
                if n_step == self.env._max_steps or self.env.is_state_over_bounds(
                    next_state
                ):
                    running = False
                    continue
